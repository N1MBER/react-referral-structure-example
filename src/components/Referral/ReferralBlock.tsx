...
const ReferralBlock:FC = () => {
    ...
    return (
        <div className={style.referralBlock}>
            <div className={style.referralBlock__links}>
                {getPages().map((page, index) =>
                    <button
                        className={classNames(style.referralBlock__link, {
                            [style.active]: activePage === index + 1
                        })}
            ...
            <button
                onClick={() => copyLink()}
                className={classNames(style.referralBlock__button, {
                    [style.message]: viewMessage
                })}
            >
                {ReferralIcon}
                <p className={style.referralBlock__button_blue}>Копировать Реферальную ссылку</p>
            </button>
        </div>
    )
}
